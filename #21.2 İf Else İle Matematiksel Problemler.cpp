#include <stdio.h>
#include <math.h> // sqrt() fonksiyonu icin kullanilacaktir
...
int main()
{
	...
	if(diskriminant > 0)
	{
		...
		printf("Iki adet ayrik ve gercel kok bulunmaktadir: %.2f and %.2f", kok1, kok2);
	}
	else if(diskriminant == 0)
	{
		kok1 = kok2 = -b / (2 * a);
		
		printf("Birbirlerine esit iki adet gercel kok bulunmaktadir: %.2f and %.2f", kok1, kok2);	
	}
	else if(diskriminant < 0)
	{
		kok1 = kok2 = -b / (2 * a);
		...
		printf("Iki adet ayrik ve karmasik kok bulunmaktadir: %.2f + i%.2f ve %.2f - i%.2f",
